...
namespace impl {
    ...
    T miller_rabin(mint a, T d, int r) {
        ...
        while (r--) {
            mint y = x * x;
            if (y == one)
                return std::__gcd(x.val() - 1, mint::mod());
            x = y;
            if (x == mone)
                return 1;
        }
    ...
    static u64 prime_or_factor_64(u64 n) {
        ...
        if (n % 2 == 0)
            return 2;
        u64 g = std::__gcd(n, 0xe221f97c30e94e1dull);
        if (g != 1)
            return g != n ? g : 0;
...
namespace impl {
    ...
    struct MontgomeryCurve {
        struct Point {
            mint x, z;
            T check() const {
                return std::__gcd(z.val(), mint::mod());
            }
        };
    ...
    T ecm_modint() {
        ...
        while (true) {
            ...
            for (int i = B1 / 6 * 6; i < B2; i += 6) {
                ...
                if (i % 300 == 0) {
                    f = std::__gcd(xprod.val(), mint::mod());
                    if (f != 1) {
                        if (f != mint::mod())
                            return f;
                        break;
