...
namespace pdqsort_detail {
    ...
    inline T* align_cacheline(T* p) {
        auto ip = reinterpret_cast<usize>(p);
    ...
    inline std::pair<Iter, bool> partition_right_branchless(Iter begin, Iter end, Comp comp) {
        ...
        if (!already_partitioned) {
            ...
            while (first < last) {
                ...
                usize left_split = !num_l ? (!num_r ? num_unknown / 2 : num_unknown) : 0;
                usize right_split = !num_r ? (num_unknown - left_split) : 0;
            ...
            if (num_r) {
                offsets_r += start_r;
                while (num_r--)
                    iter_swap(offsets_r_base - offsets_r[num_r], first), ++first;
                last = first;
            }
        }
    ...
    template <typename Iter, typename Comp, bool Branchless = is_default_compare_v<std::decay_t<Comp>> && std::is_arithmetic_v<typename std::iterator_traits<Iter>::value_type>>
    inline void pdqsort_loop(Iter begin, Iter end, Comp comp, int bad_allowed, bool leftmost = true) {
        loop {
            ...
            else if (already_partitioned && partial_insertion_sort(begin, pivot_pos, comp) && partial_insertion_sort(pivot_pos + 1, end, comp))
                return;
...
template <typename Iter, typename Comp>
void pdqsort(Iter begin, Iter end, Comp comp) {
    if (begin == end)
        return;
...
template <typename Iter>
void pdqsort(Iter begin, Iter end) { pdqsort(begin, end, std::less{}); }
template <typename Iter, typename Comp>
void pdqsort_branchless(Iter begin, Iter end, Comp comp) {
    if (begin == end)
        return;
...
template <typename Iter>
void pdqsort_branchless(Iter begin, Iter end) { pdqsort_branchless(begin, end, std::less{}); }